...
# 生成每个用户的画像特征（all_topic_fav_7），模拟不同用户有不同偏好
user_profiles = {}
for uid in range(num_users):
    num_fav = random.randint(2, 5)
    topics = random.sample(range(1, num_topics+1), num_fav)
    weights = [round(random.uniform(0.1, 0.9), 4) for _ in range(num_fav)]
    # 格式： "id1:weight1,id2:weight2,..."
    fav_str = ','.join(f"{t}:{w}" for t, w in zip(topics, weights))
    user_profiles[uid] = fav_str

data = []
for _ in range(num_rows):
    client_id = random.randint(0, num_users-1)
    client_type = random.choice([0,1,2,3])
    post_type = random.randint(0, 1)
    topic_id = random.randint(1, num_topics)
    follow_topic_id = random.randint(1, num_topics)
    all_topic_fav_7 = user_profiles[client_id]
    keyword_id = random.randint(1, num_keywords)

    # 生成 act 标签：简单规则，若 topic_id 在用户画像中出现，则更可能为正
    profile_topics = [int(pair.split(':')[0]) for pair in all_topic_fav_7.split(',')]
    if topic_id in profile_topics:
        act = 1 if random.random() < 0.7 else 0
    else:
        act = 1 if random.random() < 0.3 else 0

    data.append([act, client_id, client_type, post_type, topic_id,
                 follow_topic_id, all_topic_fav_7, keyword_id])

# ========== 3. 保存为 TSV 文件（制表符分隔） ==========
df = pd.DataFrame(data, columns=COL_NAME)
df.to_csv('user_item_act_test.csv', sep='\t', index=False, header=False)

print("数据已生成并保存为 user_item_act_test.tsv (制表符分隔，无表头)")
print("前 5 行示例：")
